...
const ContractorCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            contractorName: _entity?.contractorName,
...
pegawaiPenguasaVal: _entity?.pegawaiPenguasaVal,
        ...
        const result = await client.service("contractor").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info contractor updated successfully" });
    ...
    return (
        <Dialog header="Edit Contractor" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="contractor-edit-dialog-component">
