...
import contractorFakerFactory from "./contractorFakerFactory";
...
const ContractorFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contractorFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contractor").create(elem));
...
export default connect(mapState, mapDispatch)(ContractorFakerDialogComponent);
