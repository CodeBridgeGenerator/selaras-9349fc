...
import IssueFakerDialogComponent from "./IssueFakerDialogComponent";
import IssueSeederDialogComponent from "./IssueSeederDialogComponent";
...
const IssuePage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Issue", type: "error", message: error.message || "Failed get Issue" });
            });

    const service = _.find(config.services, { serviceName : "issue"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("issue").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Issue", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("issue").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Issue </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="issue-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="issue-datatable">
                    <IssueDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <IssueEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <IssueCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <IssueFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <IssueSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(IssuePage);
