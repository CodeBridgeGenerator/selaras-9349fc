...
import issueFakerFactory from "./issueFakerFactory";
...
const IssueFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = issueFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("issue").create(elem));
...
export default connect(mapState, mapDispatch)(IssueFakerDialogComponent);
