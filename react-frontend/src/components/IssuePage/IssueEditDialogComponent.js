...
issueName: _entity?.issueName,
priorityStatus: _entity?.priorityStatus,
issueOwner: _entity?.issueOwner,
        };

        setLoading(true);
        try {
            
        const result = await client.service("issue").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info issue updated successfully" });
    ...
    return (
        <Dialog header="Edit Issue" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="issue-edit-dialog-component">
