...
import riskFakerFactory from "./riskFakerFactory";
...
const RiskFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = riskFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("risk").create(elem));
...
export default connect(mapState, mapDispatch)(RiskFakerDialogComponent);
